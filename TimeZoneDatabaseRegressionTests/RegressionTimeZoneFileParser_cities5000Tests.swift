...
class RegressionTimeZoneFileParser_cities5000Tests: XCTestCase {
    ...
    func testInit_valid_cities5000() {
        self.measureBlock() {
    ...
    func testParseLocations_valid_cities5000() {
        let parser = try? self.timeZoneFileParser()
        self.measureBlock { () -> Void in
            let locations = parser?.parseLocations()
            XCTAssertEqual(locations?.count, 31091, "28446 cities should be parsed from cities5000 file")
        }
    }
